...
{
    int sock;  
    struct sockaddr_in serverG_add, clientS_add;
	...
	/* inizializzazione indirizzo del serverG */
    //SIN_FAMILY
    ...
    /* si stabilisce la connessione col serverG */
    //CONNECT
    if (connect(sock, (struct sockaddr *) &serverG_add, sizeof(serverG_add)) < 0) {
    fprintf(stderr,"connect error\n");
    exit(1);
  }
    /* lancio della funzione dove verranno eseguite operazioni di lettura e scrittura */
...
void VerificaGreenPass(FILE * filein, int socket)
{
        //dichiarazione dei pacchetti per l'invio e la ricezione dei dati
	...
	invioServerG.ID = 2;
	 ...
	 //codice fiscale preso da linea di comando
	   ...
	   //lettura della variabile validita per verificare se il green pass è valido
	   printf("I dati ricevuti dal serverG sono i seguenti:\n");
	   if(ricezioneServerG.validita == 1)
	   	printf("il tuo greenpass è valido per altri %d mesi\n", ricezioneServerG.mesiGreenPass);
	   else if(ricezioneServerG.validita == 0)
		printf("il tuo greenpass è scaduto\n");	
	    }
}
